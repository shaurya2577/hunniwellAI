...
Usage (from repo root):
  python -m platforms.innovator.run_downloader           # full auto: grab all companies
  python -m platforms.innovator.run_downloader --test-one # process only first company
  python -m platforms.innovator.run_downloader --manual  # manual: navigate, press Enter per company
  python -m platforms.innovator.run_downloader --save-storage   # one-time: save auth
"""
import logging
import os
import re
...
from platforms.innovator import config
...
def run_auto_loop(page, context, test_one: bool = False) -> None:
    ...
    """
    from platforms.innovator.recordings import macro as macro_module
